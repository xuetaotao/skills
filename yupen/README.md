...
- **鱼盆模型策略**：基于20日均线判断指数强弱状态，结合偏离度生成5级交易信号
- **状态转变标记**：当指数YES/NO状态在当日发生转变时，HTML报告中会以⚡标记高亮提醒
- **多格式报告**：支持JSON、Markdown、HTML三种输出格式；趋势强度排名按A股/美股/大宗商品分组，并附各组数据更新时间；可通过开关控制"数据源/PE百分位"列是否展示；报告标题栏仅展示刷新时间
...
编辑 `yupen/src/config.py` 中的 `REPORT_CONFIG["显示数据源PE指标"]`：

- `True`：趋势强度排名显示"数据源/PE百分位"列
- `False`：趋势强度排名隐藏该列
