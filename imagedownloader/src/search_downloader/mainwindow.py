...
from . import image_downloader_cli as image_downloader
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.ui.actionAbout.triggered.connect(self.dialog_about.show)

        self.ui.pushButton_load_file.clicked.connect(
            lambda: self.ui.lineEdit_path2file.setText(QFileDialog.getOpenFileName(
                self, "Load keywords from file", "./", "Text files (*.txt)")[0]))
        self.ui.pushButton_output.clicked.connect(
            lambda: self.ui.lineEdit_output.setText(QFileDialog.getExistingDirectory(
                self, "Set output directory", "./")))

        self.ui.pushButton_start.clicked.connect(self.start_download)
        self.ui.pushButton_cancel.clicked.connect(self.cancel_download)
        
        # Create convenience shortcuts to UI elements for backward compatibility
        self._setup_ui_shortcuts()

    def _setup_ui_shortcuts(self):
        """Create shortcuts to frequently used UI elements"""
        self.progressBar_current = self.ui.progressBar_current
        self.progressBar_total = self.ui.progressBar_total
        self.plainTextEdit_log = self.ui.plainTextEdit_log
        self.label_time_elapsed = self.ui.label_time_elapsed
        self.radioButton_google = self.ui.radioButton_google
        self.radioButton_bing = self.ui.radioButton_bing
        self.radioButton_baidu = self.ui.radioButton_baidu
        self.radioButton_chrome_headless = self.ui.radioButton_chrome_headless
        self.radioButton_chrome = self.ui.radioButton_chrome
        self.radioButton_api = self.ui.radioButton_api
        self.radioButton_http = self.ui.radioButton_http
        self.radioButton_socks5 = self.ui.radioButton_socks5
        self.lineEdit_output = self.ui.lineEdit_output
        self.lineEdit_path2file = self.ui.lineEdit_path2file
        self.lineEdit_keywords = self.ui.lineEdit_keywords
        self.lineEdit_proxy = self.ui.lineEdit_proxy
        self.checkBox_face_only = self.ui.checkBox_face_only
        self.checkBox_safe_mode = self.ui.checkBox_safe_mode
        self.checkBox_proxy = self.ui.checkBox_proxy
        self.checkBox_from_file = self.ui.checkBox_from_file
        self.spinBox_max_number = self.ui.spinBox_max_number
        self.spinBox_num_threads = self.ui.spinBox_num_threads
        self.pushButton_start = self.ui.pushButton_start
        self.pushButton_cancel = self.ui.pushButton_cancel
